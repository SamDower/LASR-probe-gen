...
class LabelledDataset(BaseDataset[LabelledRecord]):
    ...
    def labels_numpy(self) -> Float[np.ndarray, " batch_size"]:  # noqa: F722
        return np.array([label.to_int() for label in self.labels])

    def labels_torch(self) -> Float[torch.Tensor, " batch_size"]:  # noqa: F722
        return torch.tensor(
...
def subsample_balanced_subset(
    ...
    if n_per_class is None:
        if include_ambiguous:
            ...
            n_per_class = min(len(negative_indices), len(positive_indices))

    try:
        indices = random.sample(negative_indices, n_per_class) + random.sample(
            positive_indices, n_per_class
        )
        if include_ambiguous:
